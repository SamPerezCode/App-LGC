...
export const useDashboardMetrics = (
  ...
  useMemo(() => {
    ...
    const completionRate = totalSeguimientos
      ? Math.round(
          (seguimientosCompletados / totalSeguimientos) *
            100
        )
      : 0;

    const totalActividades =
      actividadesRutaMock.length || 1;
    ...
    const contactos = contactosMock.map((contacto) => {
      ...
      return {
        id: contacto.id,
        title:
          persona?.nombreCompleto ?? "Persona sin nombre",
    ...
    const actividades = actividadesRutaMock.map(
      (actividad) => ({
        id: actividad.id,
    ...
    contactosMock.forEach((contacto) => {
      ...
      const current = lastContactByPersona.get(
        contacto.personaId
      );
      if (!current || timestamp > current) {
        lastContactByPersona.set(
          contacto.personaId,
          timestamp
        );
      }
    ...
    const sinContacto = personasMock.filter((persona) => {
      const lastContact =
    ...
    const routeSteps: RouteStepItem[] =
      actividadesRutaMock.map((actividad) => {
        const completadas = seguimientosMock.filter(
          (s) =>
    ...
    const kpis: KpiData[] = [
      {
        ...
        iconBgClass:
          "bg-lgc-primary/10 dark:bg-lgc-darkPrimary/20",
      ...
      },
      {
        ...
        iconBgClass:
          "bg-lgc-manna/40 dark:bg-lgc-darkSurfaceMuted",
      },
      {
        id: "ruta",
        ...
        icon: KPI_ICONS.ruta,
        iconBgClass:
          "bg-lgc-olive/30 dark:bg-lgc-darkSurfaceMuted",
        progress: clampPercent(avancePercent),
        progressClass:
          "bg-lgc-olive dark:bg-lgc-darkPrimary",
      },
      {
        id: "seguimientos",
        ...
        iconBgClass:
          "bg-lgc-danger/10 dark:bg-lgc-darkSurfaceMuted",
      },
    ];
